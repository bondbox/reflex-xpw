...
uninstall:
	python3 -m pip uninstall -y reflex-xpw
...
flake8:
	flake8 reflex_xpw --count --select=E9,F63,F7,F82 --show-source --statistics
	flake8 reflex_xpw --count --exit-zero --max-complexity=10 --max-line-length=127 --statistics
pytest:
